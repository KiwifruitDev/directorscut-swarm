...
BEGIN_VS_SHADER(PBR, "PBR shader")
    ...
    float GetAlpha(IMaterialVar** ppParams)
    {
        if (!ppParams)
        {
            ppParams = s_ppParams;
    ...
    SHADER_INIT
    {
        ...
        int envMapFlags = 0; //g_pHardwareConfig->GetHDRType() == HDR_TYPE_NONE ? TEXTUREFLAGS_PWL_CORRECTED : 0;
    ...
    SHADER_DRAW
    {
        ...
        if (IsSnapshotting())
        {
            ...
            int nShadowFilterMode = 0; //bHasFlashlight ? g_pHardwareConfig->GetShadowFilterMode() : 0;
        ...
        else // Not snapshotting -- begin dynamic state
        {
            ...
            */
            ...
            Vector flLScale = pShaderAPI->GetToneMappingScaleLinear();
            modulationColor[0] *= flLScale.x;
            modulationColor[1] *= flLScale.y;
            modulationColor[2] *= flLScale.z;
