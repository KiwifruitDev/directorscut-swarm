...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ...
    else
    {
        ...
        // DXFIXME: What does this do and how could it be re-added?
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    if (g.ActiveId == id)
    {
        if (g.ActiveIdSource == ImGuiInputSource_Mouse)
        {
            ...
            {
                g.ActiveIdClickOffset.x = g.IO.MousePos.x - bb.Min.x;
                g.ActiveIdClickOffset.y = g.IO.MousePos.y - bb.Min.y;
            }
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImVec2 textPosMin;
    ImVec2 textPosMax;
    textPosMin.x = bb.Min.x + style.FramePadding.x;
    textPosMin.y = bb.Min.y + style.FramePadding.y;
    textPosMax.x = bb.Max.x - style.FramePadding.x;
    textPosMax.y = bb.Max.y - style.FramePadding.y;
    RenderTextClipped(textPosMin, textPosMax, label, NULL, &label_size, style.ButtonTextAlign, &bb);
...
void ImGui::Image(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col)
{
    ...
    if (border_col.w > 0.0f)
    {
        ...
        ImVec2 imageVecMin;
        ImVec2 imageVecMax;
        imageVecMin.x = bb.Min.x + 1;
        imageVecMin.y = bb.Min.y + 1;
        imageVecMax.x = bb.Max.x - 1;
        imageVecMax.y = bb.Max.y - 1;
        window->DrawList->AddImage(user_texture_id, imageVecMin, imageVecMax, uv0, uv1, GetColorU32(tint_col));
    }
    else
    {
